...
void printtxt(deque<student>& info, int amount, student& stud, string ammount) {
    ...
    deque<student> kietiakaii;
    deque<student> varguoliaii;
        ...
        if (strategija == 1) {
           ...
           */
           // /*
            ...
            std::deque<std::remove_reference<decltype(info)>::type::value_type> varguoliaii(std::make_move_iterator(partition_it), std::make_move_iterator(info.end()));
