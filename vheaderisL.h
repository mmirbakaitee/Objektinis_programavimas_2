...
void addstudent(list<student>& students, student& stud) {
    ...
    if (choice == 'p' || choice == 'P') {
        ...
        for (auto& student : students) {
            student.avg = getavg(student.p);
            student.median = getmedian(student.p);
        }
    }
    else {
        student info;
...
double getmedian(const list<int>& p) {
    list<int> sortedp(p);
    sortedp.sort();
    auto mid = std::next(sortedp.begin(), sortedp.size() / 2);
    if (sortedp.size() % 2 == 0) {
        auto prev = std::prev(mid);
        return static_cast<double>(*prev + *mid) / 2.0;
    }
    else {
...
void printtxt(list<student>& info, int amount, student& stud, string ammount) {
    ...
    list<student> kietiakaii;
    list<student> varguoliaii;
    string vardas, pavarde;
    char eil[100];
    int gradeam = 0; int strategija;
    ...
    getline(fq, row);
        ...
        if (strategija == 1) {
            ...
            for (int i = 0; i < amount; i++) {
                if (stud.avg * 0.4 + stud.e * 0.6 >= 5.00) {
                    kietiakaii.push_back(stud);
                }
                else {
                    varguoliaii.push_back(stud);
                }
            }
         ...
         //  */
            /*
            auto partition_it = std::partition(info.begin(), info.end(), [](const auto& i) { return i.avg * 0.4 + i.e * 0.6 < 5.00; });
            std::list<student> varguoliaii(std::make_move_iterator(partition_it), std::make_move_iterator(info.end()));
            info.erase(partition_it, info.end());
            */
