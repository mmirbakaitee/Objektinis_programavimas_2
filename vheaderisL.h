...
void printtxt(list<student>& info, int amount, student& stud, string ammount) {
    ...
    getline(fq, row);
        ...
        //  cout << left << "VISAS LAIKAS:                 " << left << durationkur.count() / 1000000.0 + durationnus.count() / 1000000.0 + durationrus.count() / 1000000.0 + durationkiet.count() / 1000000.0 + durationvarg.count() / 1000000.0 << " seconds. " << endl;               
        cout << endl;
